...
desc_terms dt,
core_desc_terms cdt,
( select distinct dq.core_desc_qualifierid_fk as qid,
  cdq.desc_qualifier as qual_text,
  dq.desc_qualifierid_pk as desc_qual_pk
  from
  desc_qualifiers dq,
  core_desc_qualifiers cdq
  where dq.core_desc_qualifierid_fk = cdq.core_desc_qualifierid_pk
) v_qual,
( select distinct cdcv.core_desc_cv as ctrl_val_text,
  dcv.desc_cvid_pk,cdcv.core_desc_cvid_pk,
  cdcv.core_desc_cv_source as source
  from
  core_desc_control_values cdcv,
  desc_control_values dcv
  where cdcv.core_desc_cvid_pk = dcv.core_desc_cvid_fk
) ctrl_values
WHERE
...
for i, g in df_ora.groupby("TERM"):
    g.drop("TERM", axis=1).to_csv(
        "{}.tsv".format(i.replace(" ", "_").split("/")[0]), index=False, sep="\t"
    )
