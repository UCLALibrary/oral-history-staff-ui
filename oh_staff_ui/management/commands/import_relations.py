...
class Command(BaseCommand):
    help = "Imports ProjectItem relation data from a TSV file"
    ...
    def handle(self, *args, **options):
        ...
        print(f"Found {len(dicts_list)} rows of relations to import.")

        total_skipped = 0
        for row in dicts_list:
            if not row["VALUE"]:
                print(f"Empty Relation for ARK {row['ARK']}. Row ignored.")
                total_skipped += 1
                continue
            ...
            item.relation = row["VALUE"]
            item.save()

        print("Finished importing relations.")
